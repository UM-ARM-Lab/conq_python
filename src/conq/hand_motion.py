...
from bosdyn.client.frame_helpers import get_a_tform_b, ODOM_FRAME_NAME, GRAV_ALIGNED_BODY_FRAME_NAME, HAND_FRAME_NAME
...
def randomized_look(command_client, robot_state_client, callback: Callable, x, y, z, pitch, yaw):
    dx = 0
    dy = 0
    dpitch = 0
    done = False
    while not done:
        look_cmd = hand_pose_cmd(robot_state_client, x + dx, y + dy, z, 0, pitch + dpitch, yaw,
                                 duration=1)
